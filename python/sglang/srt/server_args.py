...
class ServerArgs:
    ...
    # worker_pool_model_configs: Dict[str, Dict[str, Any]] = field(default_factory=dict)
    ...
    # for redis generate and response queue
    generate_request_key_prefix: str = "generate_request"
    # for redis generation request queues
    queue_id: Optional[str] = None
    frontend_generate_request_key_prefix: Optional[str] = None
    backend_generate_request_key_prefix: Optional[str] = None
    engine_to_gpu_scheduler_key_prefix: Optional[str] = None
    ...
    def __post_init__(self):
        ...
        if self.enable_worker_pool:
            ...
            # for model_id, config in self.worker_pool_model_configs.items():
            #     required_fields = ["model_path", "tokenizer_path"]
            #     missing_fields = [field for field in required_fields if field not in config]
            #     if missing_fields:
            #         raise ValueError(f"Missing required fields in model config for {model_id}: {missing_fields}")
    ...
    def from_multi_model_server_args(
        ...
        for key in keys_to_remove:
            multi_model_server_args.pop(key, None)

        if instance_config is None:
            return ServerArgs(worker_id=worker_id, **multi_model_server_args)
        else:
            return ServerArgs(
    ...
    def add_cli_args(parser: argparse.ArgumentParser):
        ...
        # the maximum memory that model weights and KV cache can use.
        parser.add_argument(
            "--max-mem-usage",
            type=float,
            default=ServerArgs.max_mem_usage,
            help="The maximum memory that model weights and KV cache can use. If not specified, it will be automatically calculated based on the mem-fraction-static.",
        )
        parser.add_argument(
        ...
        parser.add_argument(
            "--on",
            type=bool,
            default=ServerArgs.on,
            help="Whether to turn on the model.",
        )
        parser.add_argument(
        ...
        parser.add_argument(
            "--use-kvcached-v0",
            action="store_true",
            help="Use kvcached v0 for elastic memory. kvcached version: a9c5b53647112940f67ff892ec41d2bf8ebfbdcc",
        )
        parser.add_argument(
            "--abort-exceed-slos",
            action="store_true",
        ...
        parser.add_argument(
            "--queue-id",
            type=str,
            default=ServerArgs.queue_id,
            help="The queue id for multi-model serving.",
        )
        parser.add_argument(
            "--frontend-generate-request-key-prefix",
            type=str,
            default=ServerArgs.frontend_generate_request_key_prefix,
            help="The prefix of the frontend generate request key for multi-model serving.",
        )
        parser.add_argument(
            "--backend-generate-request-key-prefix",
            type=str,
            default=ServerArgs.backend_generate_request_key_prefix,
            help="The prefix of the backend generate request key for multi-model serving.",
        )
        parser.add_argument(
            "--generate-request-key-prefix",
            type=str,
            default=ServerArgs.generate_request_key_prefix,
            help="The prefix of the Redis generate request key for multi-model serving.",
        )
        parser.add_argument(
            "--enable-worker-pool",
            action="store_true",
        ...
        parser.add_argument(
            "--workers-per-gpu",
            type=int,
            default=ServerArgs.workers_per_gpu,
            help="The number of workers per GPU for multi-model serving.",
        )
        parser.add_argument(
            "--worker-id",
            type=int,
            default=ServerArgs.worker_id,
            help="The id of the worker for multi-model serving.",
        )
        parser.add_argument(
            "--num-gpus",
            type=int,
            default=ServerArgs.num_gpus,
            help="The number of GPUs for multi-model serving.",
        )
