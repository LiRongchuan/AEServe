...
class DeactivateReqInput:
    ...
    preempt: bool = False  # whether to preempt the ongoing requests
    ...
    def __post_init__(self):
        ...
        if self.preempt_mode not in PreemptMode.__members__:
            raise ValueError(
                f"Invalid preempt mode: {self.preempt_mode}. Should be one of {PreemptMode.__members__}"
            )
